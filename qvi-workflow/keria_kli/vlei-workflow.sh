...
#    SALLY_OOBI="${SALLY_HOST}/oobi" # TODO switch to direct mode self-oobi once it is ready
    ...
    tsx "${SIG_TS_WALLETS_DIR}/qars/qars-person-single-sig-oobis-setup.ts" $ENVIRONMENT $SIGTS_AIDS $OOBIS_FOR_KERIA
...
function resolve_geda_and_LE_OOBIs() {
    ...
    tsx "${SIG_TS_WALLETS_DIR}/qars/qars-resolve-geda-and-le-oobis.ts" $ENVIRONMENT $SIGTS_AIDS $MULTISIG_OOBIS
}
resolve_geda_and_LE_OOBIs

# Create delegated multisig QVI AID
function create_qvi_multisig() {
    QVI_MULTISIG_SEQ_NO=$(tsx "${SIG_TS_WALLETS_DIR}/qars/qar-check-qvi-multisig.ts" \
      "${ENVIRONMENT}" \
      "${QVI_MS}" \
      "${SIGTS_AIDS}"
      )
