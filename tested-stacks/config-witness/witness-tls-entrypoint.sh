#!/usr/bin/env bash
# Suggested witness entrypoint script.
#
...
#
# Note: This scripts shows how to run witnesses using TLS. For non-TLS, see the witness-entrypoint.sh script.
...
function start_witness() {
  ...
  kli witness start \
    ...
    --keypath /tls-certs/tls.key \
    --certpath /tls-certs/tls.crt \
    --cafilepath /tls-certs/tls.ca-chain &
  PID=$!
  wait $PID
}
